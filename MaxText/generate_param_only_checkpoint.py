...
def _read_train_checkpoint(config, checkpoint_manager, mesh):
  ...
  Distillation training stores a sibling `teacher_params` under `state.params`.
  Restore only the student subtree (state.params['params']) to avoid Orbax
  metadata mismatches, then construct a lightweight TrainState for conversion.
  """
  # Lazily import orbax here to keep dependencies local to restore logic.
  import orbax.checkpoint as ocp  # pylint: disable=import-error

  # Model and Optimizer definition (used to derive annotations/shardings).
  ...
  # Get abstract state and mesh annotations without performing a full restore.
  abstract_state, state_mesh_notations, _ = maxtext_utils.get_abstract_state(
      model, tx, config, rng, mesh, is_training=True
  )

  # Resolve path to the 'items' directory inside the full-state checkpoint.
  # Accept either .../<step>/items or .../<step>.
  p = epath.Path(config.load_full_state_path)
  if p.name == "items":
    items_dir = p
  elif (p / "items").exists():
    items_dir = p / "items"
    ...
    raise ValueError(
        f"load_full_state_path must point to a step directory or its 'items' subdir; got: {config.load_full_state_path}"
    )

  # Restore only the 'params' tree from the full-state checkpoint 'items'.
  # This avoids matching the full TrainState structure and any extra KD fields.
  # Note: We intentionally do not request/validate teacher_params.
  ckptr = ocp.PyTreeCheckpointer(
      use_ocdbt=config.checkpoint_storage_use_ocdbt,
      use_zarr3=config.checkpoint_storage_use_zarr3,
  )
  # Construct restore args for the student's params subtree only.
  abstract_params = abstract_state.params  # {'params': ...}
  ...
  # Keep only the student params in the same envelope {'params': ...}.
  # Some checkpoints may contain additional siblings under 'params' (e.g., 'teacher_params').
  restored_params = restored["params"]
  if isinstance(restored_params, dict) and "params" in restored_params:
    restored_params = {"params": restored_params["params"]}

  # Build a minimal TrainState-like container for downstream conversion.
  state = maxtext_utils.init_decode_state(None, restored_params)
...
def generate_decode_checkpoint(config):
  ...
  # We intentionally restore only student params for KD runs; opt_state may be empty.
